    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const response = await axios.get(
          `/api/admin/dashboard/dashboardData?page=${currentPage}&limit=10`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
        ...
        const stockAlerts = data.products.data.reduce((alerts, product) => {
          if (product.stock === 0) alerts.push({ ...product, alertType: "Out of Stock" });
          else if (product.stock <= 10) alerts.push({ ...product, alertType: "Low Stock" });
          return alerts;
        }, []);
        ...
        // Update Order Data
        const orderSummary = data.data.reduce(
          (acc, order) => {
            acc.totalOrders++;
            switch (order.orderStatus) {
              case "Pending":
      ...
      case "Dashboard":
        return (
          <>
            ...
            <OrderStatistics {...orderData} />
            <Graph chartData={[dashboardData.totalUsers, dashboardData.totalProducts, dashboardData.totalOrders]} />
          </>
        );
      case "Products":
        return <Product />;
